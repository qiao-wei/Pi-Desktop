...
export function createAgentShims(dirs) {
  ...
  // Bytecode caches must not land in the shipped runtime tree; keep them under the app cache.
  // POSIX separators on purpose: this value only ever goes into the /bin/sh launchers.
  const pythonPycache = shellQuote(`${dirs.agentPythonCacheDir}/pycache`);
  ...
  function renderPosixShim(pythonBin, kind, pythonHome = "") {
    ...
    return [
      ...
      // Bytecode caches must never land in the shipped runtime tree: they bake this machine's
      // absolute paths into the bundle (and the bundle may not even be writable). Redirect them
      // to the app-managed cache dir instead of disabling caching outright.
      `export PYTHONPYCACHEPREFIX=${pythonPycache}`,
      ...resolve,
  ...
  function renderWindowsShim(pythonBin, kind, pythonHome = "") {
    ...
    return [
      ...
      'set "VIRTUAL_ENV="',
      ...resolve,
      `if not defined PYTHONPYCACHEPREFIX set "PYTHONPYCACHEPREFIX=${wPythonCache}\\pycache"`,
