...
import { collapsePanel, field } from "./surfaces";
...
function ReasoningTrigger({
  ...
  const label = duration
    ? t("process.reasoningWithDuration", { seconds: duration })
    : t("process.reasoning");
  ...
  return (
    <CollapsibleTrigger
      ...
      className={cn(
        "aui-reasoning-trigger group/trigger text-foreground/65 hover:text-foreground/95 flex max-w-full origin-left items-center gap-1.5 rounded-md py-1 text-[14px] leading-6 transition-colors outline-none",
        className,
      )}
      {...props}
    >
      ...
      <ChevronRightIcon
        ...
        className="aui-reasoning-trigger-chevron size-4 shrink-0 opacity-60 transition-transform duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] group-data-open/trigger:rotate-90 group-data-panel-open/trigger:rotate-90 motion-reduce:transition-none"
      />
    </CollapsibleTrigger>
  );
}
...
function ReasoningContent({
  ...
  return (
    <CollapsibleContent
      ...
      className={cn(
        "aui-reasoning-content text-muted-foreground relative overflow-hidden text-[14px] outline-none",
        collapsePanel,
...
function ReasoningText({
  ...
  useEffect(() => {
    ...
    // A streaming text node can grow while the content element remains at
    // its max height. In that case ResizeObserver does not fire, although
    // scrollHeight changes and the live preview still needs to follow.
  ...
  return (
    <div
      ...
      className={cn(
        "aui-reasoning-text relative z-0 max-h-64 overflow-y-auto px-4 pt-3 pb-3 leading-6 text-foreground/75 text-pretty",
