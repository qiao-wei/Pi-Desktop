      ...
      {isMounted &&
        ...
        createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            className="aui-image-zoom-overlay fade-in animate-in bg-black/85 fixed inset-0 z-[140] flex items-center justify-center p-6 duration-200"
            onClick={handleClose}
            aria-label={t("image.zoomDialog")}
          >
            <img
              data-slot="image-zoom-content"
              src={src}
              alt={label}
              className="aui-image-zoom-content fade-in zoom-in-95 animate-in max-h-[72dvh] max-w-[min(72dvw,960px)] cursor-zoom-out rounded-lg bg-black/40 object-contain shadow-[0_18px_60px_rgba(0,0,0,0.45)] duration-200"
              onClick={(e) => {
                e.stopPropagation();
                handleClose();
              }}
            ...
            <button
              ...
              className="aui-image-zoom-close border-white/25 text-white/85 hover:bg-white/15 hover:text-white focus-visible:ring-white/60 absolute end-5 top-5 z-10 flex size-10 cursor-pointer items-center justify-center rounded-full border bg-black/30 backdrop-blur-sm transition-colors focus-visible:ring-2 focus-visible:outline-none"
            >
...
function ImageContentFilterError({
  ...
  return (
    ...
    >
      ...
      <p className="text-sm font-medium">{t("image.filtered")}</p>
...
const ImageImpl: ImageMessagePartComponent = (props) => {
  ...
  // A part can carry a local path instead of a data URL (a screenshot the agent
  // just wrote); the WebView needs it proxied through the local server.
  const src = resolveMediaSrc(image, getApiBase()) ?? image;
  ...
  if (status?.type === "incomplete" && status.reason === "content-filter") {
    return (
      <ImageRoot>
        <ImageContentFilterError reason={t("image.filteredReason")} />
      </ImageRoot>
    );
  }
  ...
  if (detectMediaKind(src) === "video") {
    return (
      <ImageRoot>
        <video
          data-slot="aui_image-video"
          src={src}
          aria-label={filename || t("image.videoLabel")}
          controls
          preload="metadata"
          playsInline
  ...
  return (
    <ImageRoot>
      <ImageZoom src={src} alt={filename || t("image.contentLabel")}>
        <ImagePreview src={src} alt={filename || t("image.contentLabel")} />
      </ImageZoom>
      <ImageFilename>{filename}</ImageFilename>
    </ImageRoot>
  );
