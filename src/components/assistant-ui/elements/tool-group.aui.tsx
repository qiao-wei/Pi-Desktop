...
import { useScrollPositionLock } from "./surfaces";
...
> &
  VariantProps<typeof toolGroupVariants> & {
    ...
    defaultOpen?: boolean;
    /** See `ToolCallProps["ref"]`: lets the caller measure the block before collapsing it. */
...
function ToolGroupRoot({
  ...
  const lockScroll = useScrollPositionLock(collapsibleRef, ANIMATION_DURATION);
...
function ToolGroupTrigger({
  ...
  const label =
    count === 1
      ? t("process.toolGroupCount.one", { count })
      : t("process.toolGroupCount.other", { count });

  return (
    <CollapsibleTrigger
      data-slot="tool-group-trigger"
      className={cn(
        "aui-tool-group-trigger group/trigger flex origin-left items-center gap-1.5 py-1 text-[14px] leading-6 transition-[color,scale] active:scale-[0.98]",
    ...
    >
      ...
      <span
        ...
        className={cn(
          "aui-tool-group-trigger-label-wrapper inline-block text-start text-sm leading-6 font-medium",
    ...
    >
      <div
        className={cn(
          ...
          "group-data-[variant=ghost]/tool-group-root:mt-0.5 group-data-[variant=ghost]/tool-group-root:gap-0.5",
